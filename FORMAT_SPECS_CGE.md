...
4. Encode all groups as one CGE level:
...
| **CGE (Leiden + per-cluster LLP)** | **64** | **2.887** |

The 0.432 BPE advantage of CGE over global LLP + w=64 + Fibonacci is entirely from ordering quality, not codec choice.

---

## Parameters

`CGEParams` controls all encoding decisions:
...
`CGEStats` accumulates per-section bit counts:
...
|-----------|-------------|-----------|-------|
| STD | 2.5352 | 4.8297 | +2.2945 |
| CS | 2.5347 | 4.8351 | +2.3004 |
| CGE (K=2) | 2.3191 | 4.6246 | +2.3055 |

The ~2.3 BPE overhead comes from N+1 offset entries (~25 bits each) for 325K vertices
over 3.2M edges. For CGE, additional overhead includes the extended 2K+1 cluster offset
...
| Component | CGE K=2 (best) | CGE K=1 + 3-Way Adaptive | CGE K=1 + EF | WebGraph BV |
|-----------|-----------------|---------------------------|---------------|-------------|
...
CGE K=2 beats WebGraph BV by **0.578 BPE** (20.0% reduction). The K=2 split uses **no within-cluster LLP** — the crawl-order locality within each Leiden community is already excellent for CNR-2000, and within-cluster LLP actually hurts by +0.290 BPE (see finding #17).
...
| CGE (K=1, EF) | Leiden + per-cluster LLP | 64 | 2.887 |
| CGE (K=1, implicit ranges) | Leiden + per-cluster LLP | 64 | 2.666 |
| CGE (K=1, 3-way adaptive copy) | Leiden + per-cluster LLP | 64 | 2.4341 |
| **CGE (K=2, no LLP)** | **Leiden (crawl order within clusters)** | **64** | **2.3191** |
...
| CGE K=1, intervals, no LR split | 12.927 | 163,782,982 | +0.288 |
| **CGE K=1, intervals + LR split** | **12.485** | **158,184,071** | **−0.154** |

The LR split saves **0.442 BPE** on enwiki-2013 (3.4% reduction), pushing CGE below the WebGraph BV-HC baseline. This is a larger saving than on CNR-2000 because enwiki-2013 has higher average degree (24.1 vs 9.9) and more residuals per vertex after interval extraction, amplifying the benefit of better first-value encoding.
...
1. **Vertex ordering is the dominant factor** (+1.43 BPE advantage over unordered greedy). The two-step approach — Leiden community detection creates ~34K tight fine clusters of ~9 vertices each, then per-cluster LLP ordering makes consecutive vertices share 80–90% of neighbors — is what enables CGE to beat WebGraph.
...
7. **Zeta-3 encoding is harmful for CGE** (+0.061 to +0.099 BPE vs Fibonacci). Despite being optimal for WebGraph's residuals, zeta-3 degrades copy-blocks positions encoding — it encodes copy-block starts/gaps/lengths less efficiently than Fibonacci for the small values typical in dense clusters.
