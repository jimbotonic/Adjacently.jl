...
| 4 | 1 byte | Minor version (0 = legacy, 1 = self-describing params) |
| 5 | 1 byte | Flags byte 1: graph_type (2 bits) &#124; coding_scheme (2 bits) &#124; integer_encoding (4 bits) |
| 6 | 1 byte | Flags byte 2: option_flags (8 bits) — see below |
| 7 | 5 bytes | Number of vertices (little-endian UInt40) |

**Graph type**: 0b00 = directed, 0b01 = undirected

**Coding scheme**: 0b00 = children (stop-delimited), 0b01 = index (degree array / CGE offset table)
...
**Option flags (byte 2)** — version-dependent:
...
| 0x10–0x8F | STD (Standard greedy mode) |
| 0x90–0x9F | CS (Command Stream mode) |
| 0xA0–0xAF | CGE (Clustered Graph Encoding) |
| 0xFF | Huffman (deprecated) |

*v3.1 (minor=1) — self-describing:*

| Range | Mode |
|-------|------|
| 0x00–0x0F | Algorithm IDs (hardcoded defaults) |
| 0x10–0x4F | STD + encoded params (6 bits) |
| 0x50–0x6F | CS + encoded params (5 bits) |
| 0x70–0xFF | CGE + encoded params (mixed-radix, 144 slots) |

See `MGS_HEADER.md` for full byte 2 encoding details.
...
**Index mode encoding changes** (STD):
- `stop_deltas` forced to `false` — redundant with offset table boundaries
- `empty_prefix` forced to `false` — redundant with offset table empty detection
...
### Key Finding: Greedy Beats BV, CGE Remains Superior

The greedy approach now surpasses WebGraph BV (2.881 vs 2.897 BPE) through a combination of per-vertex adaptive encoding, larger reference windows, and micro-optimizations (compact copy prefix, VLC v2, tight intervals). However, the 0.45 BPE gap to CGE (2.434 BPE) is structural:
- CGE's **per-cluster local vertex indexing** (vertices renumbered 1…|C| within each cluster) enables much tighter intra-cluster reference encoding
...
| CGE best | Leiden+LLP | all adaptive | **2.434** |
...
The greedy encoder has surpassed WebGraph BV. Remaining opportunities to close the gap to CGE (2.434 BPE):

| Opportunity | Est. savings | Feasibility |
|-------------|-------------|-------------|
| Per-cluster local IDs | 0.10–0.15 BPE | Requires architectural change (CGE-style) |
| Per-cluster bit-matrix | 0.10–0.20 BPE | Requires architectural change (CGE-style) |
...
The remaining 0.45 BPE gap to CGE is largely structural — CGE's per-cluster local vertex indexing and hierarchical encoding enable fundamentally tighter compression that the single-pass greedy approach cannot match.
