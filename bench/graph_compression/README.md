...
## Reproduction configs (`configs/*.json`)

Every experiment driver reads its dataset, encoder and ordering settings from
`configs/*.json` instead of hard-coding them:

| file | contents |
| --- | --- |
| `datasets.json` | dataset registry: loader, path, whether `get_core` applies, and for LAW graphs the published `bitsperlink`/nodes/arcs to check a download against |
| `encoders.json` | per-encoder defaults plus per-dataset overrides, and the CG K=2 configuration with its provenance |
| `orderings.json` | the ordering pipelines, including per-dataset small-cluster-merge thresholds |

**Why this exists.** Several published numbers were, at one point, reproducible
only from settings that lived outside any driver. The CG cell for cnr-2000
(2.329) could not be regenerated by any script in this directory until the
configuration was recovered from `test/run_tests_index_mode.jl`. Worse, the
`Orig.` row for cnr-2000 and in-2004 was measured on an LLP re-ordering rather
than the distributed ordering, because those two datasets had no reproduction
driver at all — the four committed SNAP-style graphs did, and their cells stayed
correct.

`verify_cg_config.jl` asserts that `configs/encoders.json` still reproduces the
published CG cell (2.3286 bpe on cnr-2000), so the recipe cannot silently drift
out of the reproduction path again. Run it after any encoder change.

### Coverage

| paper table | driver | status |
| --- | --- | --- |
| `tab:datasets` | — | graph statistics only |
| `tab:ord-ablation` | `ord_ablation.jl` (4 committed) / `ord_ablation_all.jl` (all 6, incl. LAW crawls) | covered |
| `tab:transfer` | `transfer.jl` | covered |
| `tab:highlight` | `sota_wholegraph.jl` | covered |
| `tab:ra-sota` | `sota_ra.jl` | covered |
| `tab:ablation` | `feature_ablation.jl` | **published table does not reproduce; three rows have the wrong sign and the endpoint is unreachable — see the driver's header** |
| `tab:bvbudget` | — | read from LAW's `.properties` |
