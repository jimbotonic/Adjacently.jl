...
```
Header: 4D 47 53 03 01 02 00 04 00 00 00 00
...
```
Header: 4D 47 53 03 01 12 00 04 00 00 00 00
...
This is the baseline against which compressed formats (STD, CS, CGE) are measured.
...
| **STD/CS** | 4-bit VLC stream header + bitstream-encoded vertex records |
| **CGE** | Cluster offset table (index mode) or direct bitstream + clustered encoding |

The option_flags byte (byte 2) distinguishes the format:
- `0x00`: Uncompressed (ALG_LEGACY_MGS)
- `0x02`: STD with defaults
- `0x03`: CS with defaults
- `0x04`: CGE with defaults
- `0x10-0xFF`: STD/CS/CGE with explicit parameters

See `MGS_HEADER.md` for full byte 2 encoding, and `FORMAT_SPECS_STD.md`,
`FORMAT_SPECS_CS.md`, `FORMAT_SPECS_CGE.md` for compressed format specifications.
