...
#
# Test CGE multi-level compression on CNR-2000 and measure BPE
#
...
using Adjacently
using Adjacently.CGE: encode_level, CGEParams, CGEStats, decode_level, load_cge_graph
...
function run_cge(g, m_original, params; K::Int=8, llp_passes::Int=5, ordering::Symbol=:llp)
    max_levels = 5
    min_clusters = 32
    cur_g = g
    total_bytes = 0
    ...
    println("\nStarting multi-level CGE encoding (K=$K, max_levels=$max_levels)...")
...
# Main
println("=" ^ 70)
println("CGE CNR-2000 Multi-Level Compression Test")
println("=" ^ 70)
...
configs = [
    ("FW64 K=2 LLP (ref)",   fw64_base(), "cge_fw64_k2",        2, 5, :llp,       g),
    ("FW64 K=1 LLP",         fw64_base(), "cge_fw64_k1",        1, 5, :llp,       g),
    ("FW64 K=2 Bisection",   fw64_base(), "cge_fw64_k2_bisect", 2, 5, :bisection, g),
    ("FW64 K=1 Bisection",   fw64_base(), "cge_fw64_k1_bisect", 1, 5, :bisection, g),
]
...
for (name, params, suffix, K_val, llp_val, ord_val, input_g) in configs
    ...
    all_level_bytes, total_bytes, first_level_bytes = run_cge(input_g, m_original, params; K=K_val, llp_passes=llp_val, ordering=ord_val)

    output_file = joinpath(@__DIR__, "..", "datasets", "webgraph", "cnr-2000", "cnr2000_$(suffix).cge")
...
println("\n  Step 3: CGE encode with implicit_ranges membership...")
...
output_file_impl = normpath(joinpath(@__DIR__, "..", "datasets", "webgraph", "cnr-2000", "cnr2000_cge_gllp_implicit.cge"))
