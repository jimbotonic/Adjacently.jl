...
#
# Load Arxiv_HEP-PH core & rcore, predict CGE params with trained GNN,
# grid-search for best params, further train GNN, save best .mgz files.
#
# Usage:
#   julia --project scripts/compress_arxiv_hep_ph.jl
#
...
const DS_DIR = joinpath(PROJECT_ROOT, "datasets", "Arxiv_HEP-PH")
const CORE_MGS = joinpath(DS_DIR, "Arxiv_HEP-PH_core.mgs")
const RCORE_MGS = joinpath(DS_DIR, "Arxiv_HEP-PH_rcore.mgs")
...
function main()
    fp("=" ^ 72)
    fp("  Arxiv HEP-PH CGE Compression Pipeline")
    fp("=" ^ 72)

    # =====================================================================
    # 1. Load graphs
    # =====================================================================
    fp("\n[1/7] Loading Arxiv HEP-PH core and rcore from .mgs files...")
    ...
    # =====================================================================
    fp("\n[2/7] GNN prediction on subgraphs...")
    ...
    function safe_compute_bpe(g, params, clusters, label)
        try
            t = time()
            bpe = compute_bpe(g, params, clusters)
            fp("  $label $(round(bpe, digits=4)) BPE ($(round(time()-t, digits=1))s)")
            return bpe
        catch e
            fp("  $label SKIPPED ($(typeof(e)): $(sprint(showerror, e)))")
            return Inf
        end
    end

    bpe_core_k1_gs  = safe_compute_bpe(g_core,  gs_params_core,  clusters_core_k1,  "Core K=1 (grid-search): ")
    bpe_core_k1_bl  = safe_compute_bpe(g_core,  baseline_params, clusters_core_k1,  "Core K=1 (baseline):    ")
    bpe_rcore_k1_gs = safe_compute_bpe(g_rcore, gs_params_rcore, clusters_rcore_k1, "RCore K=1 (grid-search):")
    bpe_rcore_k1_bl = safe_compute_bpe(g_rcore, baseline_params, clusters_rcore_k1, "RCore K=1 (baseline):   ")
    ...
    bpe_core_k2_gs  = safe_compute_bpe(g_core,  gs_params_core,  clusters_core_k2,  "Core K=2 (grid-search): ")
    bpe_core_k2_bl  = safe_compute_bpe(g_core,  baseline_params, clusters_core_k2,  "Core K=2 (baseline):    ")
    bpe_rcore_k2_gs = safe_compute_bpe(g_rcore, gs_params_rcore, clusters_rcore_k2, "RCore K=2 (grid-search):")
    bpe_rcore_k2_bl = safe_compute_bpe(g_rcore, baseline_params, clusters_rcore_k2, "RCore K=2 (baseline):   ")
    ...
    # =====================================================================
    # 6. Further train GNN with subgraphs
    # =====================================================================
    fp("\n[6/7] Further training GNN with Arxiv HEP-PH subgraphs...")
    ...
    for (label, g, best_tuple, ne_g) in [
        ...
        # Override non-encoded params to match decoder hardcoded defaults.
        # The header only stores: membership, window, interval_mode, mil.
        params = CGEParams(;
            L=128, varint=:fibonacci, count_varint=:fibonacci, gap=:fibonacci,
            degree=:elias_delta, membership=params.membership,
            inter_strategy=:lists,
            intra_ref_enabled=true, intra_ref_window=params.intra_ref_window,
            intra_intervals=params.intra_intervals, intra_mil=params.intra_mil,
            intra_adapt_mil=params.intra_mil, intra_lr_split=params.intra_lr_split,
        ...
        base = joinpath(DS_DIR, "Arxiv_HEP-PH_$(label)_cge_k$(K)")
        mgz_file = base * ".mgz"
    ...
    for (label, g, best_tuple, ne_g) in [
        ...
        config_name, bpe, params, clusters, K = best_tuple
        mgz_file = joinpath(DS_DIR, "Arxiv_HEP-PH_$(label)_cge_k$(K).mgz")
