...
#
# Load EAT core & rcore, predict CGE params with trained GNN
# (on subgraphs), grid-search for best params, further train GNN,
# save best .mgz files.
#
# Usage:
#   julia --project scripts/compress_eat.jl
#
...
const DS_DIR = joinpath(PROJECT_ROOT, "datasets", "EAT")
const CORE_MGS = joinpath(DS_DIR, "EAT_core.mgs")
const RCORE_MGS = joinpath(DS_DIR, "EAT_rcore.mgs")
...
function main()
    fp("=" ^ 72)
    fp("  EAT CGE Compression Pipeline")
    fp("=" ^ 72)

    # =====================================================================
    # 1. Load graphs
    # =====================================================================
    fp("\n[1/7] Loading EAT core and rcore from .mgs files...")
    ...
    # EAT is small (7754 vertices) — use 2000-vertex subgraphs for GNN
    ...
    # =====================================================================
    # 6. Further train GNN with EAT subgraphs
    # =====================================================================
    fp("\n[6/7] Further training GNN with EAT subgraphs...")
    ...
    for (label, g, best_tuple, ne_g) in [
        ...
        # Everything else must match decode_cge_params() defaults.
        params = CGEParams(;
        ...
        base = joinpath(DS_DIR, "EAT_$(label)_cge_k$(K)")
        mgz_file = base * ".mgz"
    ...
    for (label, g, best_tuple, ne_g) in [
        ...
        config_name, bpe, params, clusters, K = best_tuple
        mgz_file = joinpath(DS_DIR, "EAT_$(label)_cge_k$(K).mgz")
