...
#!/usr/bin/env julia
#
# Compress enwiki-2013 with CGE best known parameters
#
...
g_cge = relabel_graph(g, cge_vertex_map)
m_cge = ne(g_cge)
...
k_suffix = K == 1 ? "" : "_k$K"
cge_base = joinpath(DS_DIR, "$(PREFIX)_cge$(k_suffix)")
cge_mgz = cge_base * ".mgz"

@info "Encoding CGE K=$K → $cge_mgz"
t_encode_start[] = time()

write_cge_mgs3_graph(g_cge, cge_base, clusters_impl;
...
if nv(g_loaded) == nv(g_cge) && ne(g_loaded) == ne(g_cge)
    @info "  Roundtrip: PASSED ($(nv(g_loaded)) vertices, $(ne(g_loaded)) edges, decode $(dt_dec)s)"
else
    @warn "  Roundtrip: MISMATCH! Expected $(nv(g_cge))v/$(ne(g_cge))e, got $(nv(g_loaded))v/$(ne(g_loaded))e"
end

# ── Summary ──────────────────────────────────────────────────────────────────

println("\n" * "=" ^ 70)
println("SUMMARY — $DS CGE K=$K")
println("=" ^ 70)
println("  CGE K=$K:   $bpe BPE  ($(filesize(cge_mgz)) bytes)")
