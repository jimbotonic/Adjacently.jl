...
#!/usr/bin/env julia
#
# Verify roundtrip of enwiki2013_cge.mgz (CGE K=1 with LLP ordering)
#
...
cge_mgz = joinpath(DS_DIR, "$(PREFIX)_cge.mgz")
llp_path = joinpath(DS_DIR, "$(PREFIX)_llp_p10.bin")

isfile(cge_mgz) || error("CGE file not found: $cge_mgz")
...
@info "Loading compressed file: $cge_mgz"
t_dec = time()
...
println("\n" * "=" ^ 70)
println("SUMMARY — $DS CGE K=1 (LLP p=10)")
println("=" ^ 70)
println("  CGE K=1 LLP: $bpe BPE  ($(filesize(cge_mgz)) bytes)")
