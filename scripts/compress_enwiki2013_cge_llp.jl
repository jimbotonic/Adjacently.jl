...
#!/usr/bin/env julia
#
# Compress enwiki-2013 with CGE K=1 + global LLP ordering (memory-lean)
#
...
t_enc = time()
write_cge_mgs3_graph(g, cge_base, clusters; params=cge_params)
...
println("\n" * "=" ^ 70)
println("SUMMARY — $DS CGE K=1 (LLP p=$LLP_PASSES)")
println("=" ^ 70)
