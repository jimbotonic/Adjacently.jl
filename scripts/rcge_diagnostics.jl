...
#
# RCGE Diagnostics for CNR-2000
#
# Runs RCGE encoding with the best config (zigzag + stop_deltas + copy_blocks)
...
# ---------------------------------------------------------------------------
# Minimal TestGraph for coarse weighted graph (reused from test_rcge_cnr2000.jl)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Diagnostic RCGE run: encode + collect per-vertex reference stats
# ---------------------------------------------------------------------------
function run_rcge_diagnostics(g, m_original, params; K::Int=8, llp_passes::Int=5)
    max_levels = 5
    min_clusters = 32
    cur_g = g
    total_bytes = 0
    ...
    println("\nStarting RCGE diagnostic encoding (K=$K, max_levels=$max_levels)...")
            ...
            for (idx_local, u) in enumerate(C)
                ...
                if params.intra_ref_enabled && !isempty(prev_lists)
                    ...
                    if params.intra_stop_deltas
                        Adjacently.RCGE._write_stop_delta_zigzag(w_raw, UInt32.(nl), :fibonacci, _zz_vid)
                    else
                    ...
                    for rix in wstart:length(prev_lists)
                        ...
                        if params.intra_copy_blocks
                            Adjacently.RCGE._write_copy_blocks(w_ref, positions, params.varint)
                        else
                        ...
                        if params.intra_stop_deltas
                            Adjacently.RCGE._write_stop_delta_zigzag(w_ref, UInt32.(adds), :fibonacci, _zz_vid)
                        else
...
function print_stats_summary(stats::RCGEStats, info)
    mcur = info.mcur
...
# ---------------------------------------------------------------------------
println("=" ^ 70)
println("RCGE CNR-2000 Diagnostics")
println("=" ^ 70)
...
all_stats, all_diag, level_info, total_bytes = run_rcge_diagnostics(g, m_original, params; K=8, llp_passes=5)
