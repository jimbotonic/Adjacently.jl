#!/usr/bin/env julia
#
# Compress enwiki-2013 with RCGE K=1 (no clustering, pure reference coding)
#
...
println("=" ^ 70)
println("RCGE compression — $DS  K=1 (no clustering)")
println("=" ^ 70)
...
write_rcge_mgs3_graph(g, rcge_base, clusters;
...
println("\n" * "=" ^ 70)
println("SUMMARY — $DS RCGE K=1")
println("=" ^ 70)
println("  RCGE K=1:    $bpe BPE  ($(filesize(rcge_mgz)) bytes)")
println("  RCGE K=6:    16.9861 BPE (previous)")
