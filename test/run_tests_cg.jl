...
@testset "CG basic encode_level and helpers" begin
    ...
    io = IOBuffer()
    w = BitWriter(io)
    encode_level(w, g, P; params=CGParams(L=128, varint=:fibonacci, count_varint=:elias_delta, gap=:fibonacci, degree=:elias_delta, perm_strategy=:blockpos, membership=:elias_fano))
...
@testset "CG encode_level on CNR-2000 with Louvain" begin
    ...
    if !isfile(cnr_csv_path)
        ...
        params = CGParams(L=128, varint=:fibonacci, count_varint=:elias_delta, gap=:fibonacci, degree=:elias_delta, undirected_pairs=false, perm_strategy=:blockpos, membership=:elias_fano)
        t4 = time(); encode_level(w, g, clusters; params=params); flush_bitwriter(w; flush_last_bits=true); bytes = take!(io); t5 = time()
        @info "CG encode: size=$(length(bytes)) bytes, time=$(round(t5-t4,digits=3))s"
        @test length(bytes) > 0
    end
    finally
